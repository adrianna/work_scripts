...
print("Boundaries for Correction: +/- 2*stdev = [%d, %d]", correction_lb, correction_ub)
...
for site, data in file.groupby('Site'):
    ...
    if site > 0:
        ...
        empirical_offsets = data['Offset']*1e9
        predicted_offsets = data['Offset_Predict']*1e9
        corrected_offsets = data['Corrections']*1e9
        ...
        plt.hlines(correction_ub*1e9, vmin, vmax, linestyles='dashed', color='blue')
        plt.hlines(correction_lb*1e9, vmin, vmax, linestyles='dashed', color='blue')
        plt.hlines(correction_lb*1e9, vmin, vmax, linestyles='dashed', color='blue')
...
# Compare each correction value against the upper and lower bound defined limits. Values must fall within 2*sigma
f_grp = file.groupby(['Site', 'Voltage'])
temp = []
for site, volt in f_grp:
    print(site)
    #print(volt)
    within_spec = f_grp.get_group(site)['Corrections'].apply(lambda x: x > correction_lb and x <correction_ub)
    temp += [ within_spec.item() ]

file['Within_Spec'] = temp
outlier_channels = file[~file['Within_Spec']].groupby(['Site','Voltage'])
print("debug")

end = file['Voltage'][len(file['Voltage'])]
start = file['Votlage'][0]

x = file['Voltage'][start+2:end-2]
y = file['Offset'][start+2:end-2]
for site_voltage, group in outlier_channels:
    slope, intercept, r_value, p_value, std_err = stats.linregress(x, y)
    tmp = [slope] * len(f_grp.get_group(site))
    slope_array += tmp
    tmp = [intercept] * len(f_grp.get_group(site))
...
for site_voltage in outlier_channels.groups.keys():
    file[file['Site'] ==
