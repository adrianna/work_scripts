...
#        pp_scale <- pp_text + scale_y_continuous(limits=c(-0.5,0.5), breaks=seq(-0.5,0.5,0.05) )
        pp_scale <- pp_text + scale_y_continuous(limits=c(-2.5,1), breaks=c(-2,-1,-0.5,-0.25,-0.05,0,0.05,0.25,0.5,1))
        errPlot  <- pp_scale + labs(colour="pad")
...
#    if (loadboard != "DemoBoard1") {
    ...
    if (reftype == "true") {
        raw_ref_values <- read.csv(file, header=FALSE,sep=",")
        ...
        print("Printing length of select_pin")
        print(length(select_pin))
...
#    print(sum_stat)
    if (!( missing(ptitle))) {
        ...
        write.csv(sum_stat, file=pfilename, quote=FALSE, row.names=F)
    }
...
#    pad <- rep(1: dim(sumTable_t)[1])
    ...
    write.csv(sorted_sumTable, sTitle, quote=F,row.names=F)
...
#            browser()
        ...
        cdata <- cdata_ref
    } else if(identical(loadboard,charZ)) {
        print("loadboard length= 0")
    ...
    write.csv(cdata,"all_complete.csv",quote=F, row.names=F)
} else {
...
ctemp <- subset(cdata, pin!=55)
ctemp2 <- subset(ctemp, pin!=71)
cdata <-subset(ctemp2,averages==1024)
...
# Change xuB from 0.5 to 0.06 s - 1/2/15
xlB <- 0
xuB <- 0.115
top_n <- 1
...
#    browser()
    print("Printing subset settling time")
    print(unique(cds_stable$settling))
...
#        browser()
        if (pins_opt == "all") {
            ...
            print(paste("Testname: ", testname, sep=""))
            tn <- paste(testname,"_allPins", sep="")
            
            ggplotError(sumStat_pPin_pAvg,,"offset_error_norm", tn, "pad")
...
#            write.csv(sumStat_pPin_pAvg, title, quote=F)
    ...
    vtitle <- paste("./pin_stats/", (paste(paste("sumStatST_pPin_pAvg_pV",v,sep=""),"csv", sep=".")), sep="")
    write.csv(sumStat_pPin_pAvg_pVoltage, vtitle, quote=F, row.names=F)
    if (pins_opt != "all") {
...
if (pins_opt != "all") {
    ...
    write.csv(sumStat_pPin_All, "./pin_stats/sumStat_pinAll_master.csv", quote=F, row.names=F)
}
