##############################################################################
## File:    sw_current_scatter_modes.R
## Version: 1.0
## Author:  Adrianna Galletta
## Date:    4 March 2015
##
## Purpose:     Compares scatter current between fixes
...
#browser()
all_arranged <- arrange(all, mode, autorange, rng_cnt, voltage)
    ...
    for (ar in autoRng) {
        ...
        for (v in volt) {
            ...
            p <- ggplot(all_arranged[all_arranged$voltage==v & all_arranged$autorange==ar,],
                        aes(y=current)) + geom_point(aes(x=range_cnt, colour=factor(mode)) )
